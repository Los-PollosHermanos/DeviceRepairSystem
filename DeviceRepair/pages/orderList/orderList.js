...
Page({
  ...
  loadOrders: function() {
    var that = this;
    wx.request({
      url: 'http://localhost:8080/WxServlet_war_exploded/order?requestType=loadOrder&userId=1', // 将 userId 参数直接写入 URL
      method: 'GET',
  ...
  searchOrders: function() {
    ...
    wx.request({
      url: 'http://localhost:8080/WxServlet_war_exploded/order?requestType=searchOrder&remarks=' + this.data.searchQuery,
      method: 'GET',
      success: function(res) {
        that.setData({
          order_list: res.data
