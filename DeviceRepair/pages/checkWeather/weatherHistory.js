// pages/checkWeather/weatherHistory.js
...
Page({
  ...
  loadOrders: function() {
    var that = this;
    wx.request({
      url: 'http://localhost:8080/WxServlet_war_exploded/weather?requestType=loadWeather', 
      method: 'GET',
  ...
  searchOrders: function() {
    ...
    wx.request({
      url: 'http://localhost:8080/WxServlet_war_exploded/weather?requestType=searchWeather&weather_type=' + this.data.searchQuery,
      method: 'GET',
      success: function(res) {
        that.setData({
          order_list: res.data
  ...
  deleteOrder: function(e) {
    ...
    wx.showModal({
      ...
      success: function(res) {
        if (res.confirm) {
          wx.request({
            url: 'http://localhost:8080/WxServlet_war_exploded/weather?requestType=deleteWeather&orderId=' + orderId,
            method: 'POST',
            success: function(res) {
  ...
  sortById: function() {
    let sortedOrder_list = this.data.order_list.sort((a, b) => {
      return b.id - a.id;
    });

    this.setData({
      order_list: sortedOrder_list
    });
  },

  calculateStats: function() {
    let orders = this.data.order_list;
