...
public class locationServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if ("loadLocation".equals(requestType)) {
//            int userId = Integer.parseInt(request.getParameter("userId")); // 获取 URL 中的 userId 参数
            loadWeather(response);
        }
        else if ("searchLocation".equals(requestType)) {
            String city = request.getParameter("city");
            searchWeather(city, response);
        }
        else {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if ("deleteLocation".equals(requestType)) {
    ...
    private void loadWeather(HttpServletResponse response) throws IOException {
        ...
        try (Connection conn = DBConnection.getConnection();
             PreparedStatement stmt = conn.prepareStatement("SELECT * FROM wx_location")) {
            ...
            while (rs.next()) {
                ...
                order.put("time", rs.getString("time"));
                orders.put(order);
            }
    ...
    private void searchWeather(String city, HttpServletResponse response) throws IOException {
        JSONArray orders = new JSONArray();
        try (Connection conn = DBConnection.getConnection();
             PreparedStatement stmt = conn.prepareStatement("SELECT * FROM wx_location WHERE city LIKE ?")) {
    ...
    private void deleteWeather(String orderId, HttpServletResponse response) throws IOException {
        try (Connection conn = DBConnection.getConnection();
             PreparedStatement stmt = conn.prepareStatement("DELETE FROM wx_location WHERE id = ?")) {
